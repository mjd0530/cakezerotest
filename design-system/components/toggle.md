...
|-------|----------------|------------------|------------------|--------|
| **Off — default** | `false` | `--color-toggle-track-off` | `--color-toggle-thumb-off` | Map track to `primitive.slate.700`; thumb to `semantic.background.surface` (white in light theme; see §7). |
...
| **On — pressed** | `true` | `var(--color-primary)` | `--color-toggle-thumb-on-pressed` | Figma: thumb may widen with **surface** fill (`semantic.background.surface`); optional. |
...
| Thumb off (on dark track) | `semantic.background.surface` (light) / appropriate light fill (dark) | `--color-toggle-thumb-off` |
